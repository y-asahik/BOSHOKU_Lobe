...
def capture_and_predict():
    ...
    if enable_image_save and should_save:
        os.makedirs(img_dir, exist_ok=True)
        save_count += 1
        file_path = os.path.join(img_dir, f"image_{save_count:05d}_{prediction_text}_{datetime_img}.jpg")
...
# タイマーベースの撮影スレッドを開始
capture_thread = threading.Thread(target=timer_capture_loop, daemon=True)
capture_thread.start()
print(f"Timer-based capture started (interval: {capture_interval}s, save interval: {save_interval}s)")
